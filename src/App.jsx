      ...
      <div className="flex flex-wrap items-start justify-between gap-3">
        ...
        <span
          className={`rounded-full border px-3 py-1 text-xs font-semibold ${
            statusStyles[spec.status] || statusStyles.Draft
          }`}
        >
          {spec.status}
        </span>
      ...
      <div className="mt-5 grid gap-3 sm:grid-cols-2">
        ...
        <div className="rounded-2xl bg-white p-4">
          ...
          <p className="mt-1 text-sm font-medium text-slate-700">
            {spec.updated_at ? new Date(spec.updated_at).toLocaleDateString() : "—"}
          </p>
        </div>
      </div>
...
function ViewerDashboard({
  ...
  const teams = useMemo(
    () => ["All teams", ...Array.from(new Set(specs.map((s) => s.team).filter(Boolean)))],
    [specs]
  );
        ...
        <Panel title="Published opspecs" subtitle="Read-only access for company members">
          ...
          <div className="mt-6 grid gap-4 lg:grid-cols-2">
            {filteredSpecs.length ? (
              ...
              <p className="text-sm text-slate-500">No specs found.</p>
            )}
          </div>
        </Panel>
      </main>
      ...
      <main className="mx-auto grid max-w-7xl gap-6 px-6 py-8 lg:grid-cols-[0.9fr_1.1fr] lg:px-8">
        <div className="space-y-6">
          <Panel title="Create document" subtitle="Draft new opspecs for review and publishing">
            ...
            >
              <Field label="Spec ID">
                <Input value={form.id} onChange={(e) => setForm((v) => ({ ...v, id: e.target.value }))} />
              </Field>
              <Field label="Title">
              ...
              <div className="grid gap-4 md:grid-cols-2">
                <Field label="Owner">
                  <Input value={form.owner} onChange={(e) => setForm((v) => ({ ...v, owner: e.target.value }))} />
                </Field>
                <Field label="Team">
                  <Input value={form.team} onChange={(e) => setForm((v) => ({ ...v, team: e.target.value }))} />
                </Field>
                    ...
                    <div className="flex items-center justify-between gap-4">
                      <div>
                        ...
                        <p className="mt-1 text-sm text-slate-500">
                          {spec.team || "No team"} • {spec.owner || "No owner"} • {spec.version || "No version"}
                        </p>
                      </div>
                      <div className="flex items-center gap-2">
  ...
  useEffect(() => {
    ...
    async function bootstrap() {
      try {
        ...
        if (mounted) {
          setAuthError(
            "Supabase is not configured yet. Create src/lib/supabase.js and add your environment variables."
          );
        }
      } finally {
